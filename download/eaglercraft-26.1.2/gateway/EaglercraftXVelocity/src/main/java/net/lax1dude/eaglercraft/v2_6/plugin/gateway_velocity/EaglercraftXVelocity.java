package net.lax1dude.eaglercraft.v2_6.plugin.gateway_velocity;
...
/**
 * EaglercraftXVelocity - Main plugin class for the EaglerCraft gateway on Velocity.
 * <p>
 ...
 * to join Minecraft Java Edition servers through a Velocity proxy.
 * </p>
 * <p>
 * It provides feature parity with the BungeeCord version, including:
 * <ul>
 ...
 */
@Plugin(
    id = "eaglercraftxvelocity",
    name = "EaglercraftXVelocity",
    version = "1.3.2",
    description = "EaglerCraft gateway plugin for Velocity - translates WebSocket to TCP Minecraft protocol",
    authors = { "lax1dude" }
)
public class EaglercraftXVelocity {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger("EaglercraftXVelocity");

    /** Singleton plugin instance */
    private static EaglercraftXVelocity instance;

    /** Velocity ProxyServer instance */
    ...
    /**
     * Constructs the EaglercraftXVelocity plugin instance.
     * Called by Velocity's dependency injection framework.
     *
     * @param proxyServer  the Velocity ProxyServer instance
     * @param dataDirectory the plugin data directory path
     */
    @Inject
    public EaglercraftXVelocity(ProxyServer proxyServer, @DataDirectory Path dataDirectory) {
     ...
     *
     * @return the active EaglercraftXVelocity instance
     */
    public static EaglercraftXVelocity getInstance() {
        return instance;
    }

    /**
     * Gets the Velocity ProxyServer instance.
     *
    ...
    /**
     * Called when the proxy initializes. Sets up WebSocket listeners,
    ...
    public void onProxyInitialize(ProxyInitializeEvent event) {
        instance = this;
        LOGGER.info("EaglercraftXVelocity v{} starting...", PLUGIN_VERSION);
        ...
        enabled = true;
        LOGGER.info("EaglercraftXVelocity v{} enabled successfully.", PLUGIN_VERSION);
    }

    /**
     * Called when the proxy shuts down. Stops WebSocket servers,
     * disconnects players, and cleans up resources.
     *
    ...
    public void onProxyShutdown(ProxyShutdownEvent event) {
        enabled = false;
        LOGGER.info("EaglercraftXVelocity shutting down...");

        stopWebSocketServers();
        disconnectAllPlayers();
        cleanupResources();

        LOGGER.info("EaglercraftXVelocity disabled.");
    }
    ...
    public void onPostLogin(PostLoginEvent event) {
        ...
        if (session != null) {
            session.setVelocityPlayer(player);
            LOGGER.debug("EaglerCraft player session linked: {}", player.getUsername());
        }
    }
    ...
    public void onDisconnect(DisconnectEvent event) {
        ...
        if (session != null) {
            ...
            LOGGER.debug("EaglerCraft player session removed: {}", player.getUsername());
        }
    }
    ...
    private void loadConfiguration() {
        File configFile = dataDirectory.resolve("settings.yml").toFile();
    ...
    private void startWebSocketServers() {
        for (ListenerConfig lc : config.getListeners()) {
            if (!lc.isEnabled()) {
            ...
            try {
                ...
                LOGGER.info("Started WebSocket listener '{}' on {}:{}", lc.getName(), lc.getHost(), lc.getPort());
            } catch (Exception e) {
                LOGGER.error("Failed to start WebSocket listener '{}'", lc.getName(), e);
            }
        }
    }
    ...
    private void stopWebSocketServers() {
        for (Map.Entry<String, EaglerWebSocketServer> entry : wsServers.entrySet()) {
            try {
                ...
                LOGGER.warn("Interrupted while stopping WebSocket listener: {}", entry.getKey());
            }
        }
        wsServers.clear();
    }
    ...
    /**
     * Registers plugin commands with the Velocity proxy.
     */
    private void registerCommands() {
        proxyServer.getCommandManager().register("eagler", new EaglerCommand());
        proxyServer.getCommandManager().register("eaglerskin", new EaglerSkinCommand());
        proxyServer.getCommandManager().register("eaglervoice", new EaglerVoiceCommand());
    }

    /**
     * Starts periodic maintenance tasks using the Velocity scheduler.
     */
    private void startMaintenanceTasks() {
        proxyServer.getScheduler().buildTask(this, this::cleanupRateLimits)
            .repeat(60, TimeUnit.SECONDS).delay(60, TimeUnit.SECONDS).schedule();
        proxyServer.getScheduler().buildTask(this, this::validateSessions)
            .repeat(30, TimeUnit.SECONDS).delay(30, TimeUnit.SECONDS).schedule();
        proxyServer.getScheduler().buildTask(this, this::pruneVoiceChannels)
            .repeat(45, TimeUnit.SECONDS).delay(45, TimeUnit.SECONDS).schedule();
        proxyServer.getScheduler().buildTask(this, this::expireAuthCodes)
            .repeat(30, TimeUnit.SECONDS).delay(30, TimeUnit.SECONDS).schedule();
    }
    ...
    private void saveDefaultConfig(String resourcePath, File targetFile) {
        ...
        try (InputStream is = getClass().getClassLoader().getResourceAsStream(resourcePath);
            ...
            if (is == null) {
                LOGGER.error("Default resource not found: {}", resourcePath);
                return;
            }
            ...
            LOGGER.error("Failed to save default config: {}", resourcePath, e);
        }
    }
     ...
     *
     * @param serverName the Velocity registered server name
    ...
    public class EaglerWebSocketServer extends WebSocketServer {
        ...
        public void onOpen(WebSocket conn, ClientHandshake handshake) {
            ...
            LOGGER.debug("WebSocket connection opened: {} from {}", connId, ip);
        }
        ...
        public void onClose(WebSocket conn, int code, String reason, boolean remote) {
            ...
            LOGGER.debug("WebSocket connection closed: {} code={}", connId, code);
        }
            ...
            try {
                ...
                switch (type) {
                    ...
                    case "cape":
                        ...
                        break;
                    case "version":
                        handleVersionQuery(conn, json);
                        break;
                    default:
                        LOGGER.warn("Unknown text packet type: {}", type);
                }
            } catch (Exception e) {
                LOGGER.warn("Failed to parse text message", e);
            }
        }
        ...
        private void handleBinaryMessage(WebSocket conn, ByteBuffer message) {
            try {
                ...
                LOGGER.warn("Failed to handle binary message", e);
            }
        }
        ...
        private void handleMotdQuery(WebSocket conn, JsonObject json) {
            ...
            players.addProperty("max", listenerConfig.getMaxPlayers());
            players.addProperty("online", proxyServer.getPlayerCount());
            motdResponse.add("players", players);

            motdResponse.addProperty("motd", config.getMotdLine1());
            motdResponse.addProperty("motd2", config.getMotdLine2());
         ...
         *
         * @param conn the WebSocket connection
         * @param json the version query JSON object
         */
        private void handleVersionQuery(WebSocket conn, JsonObject json) {
            JsonObject resp = new JsonObject();
            resp.addProperty("type", "version");
            resp.addProperty("eagler_protocol", PROTOCOL_VERSION);
            resp.addProperty("mc_protocol", MC_PROTOCOL_VERSION);
            resp.addProperty("plugin_version", PLUGIN_VERSION);
        ...
        private void handleSkinUpload(WebSocket conn, JsonObject json) {
            ...
            if (json.has("data")) {
                ...
                LOGGER.debug("Skin registered for: {}", username);
            }
        }
        ...
        private void handleCapeUpload(WebSocket conn, JsonObject json) {
            ...
            if (json.has("data")) {
                ...
                LOGGER.debug("Cape registered for: {}", username);
            }
        }

        /**
         * Translates an EaglerCraft protocol packet and forwards it to the
         * appropriate backend Minecraft server via Velocity.
         *
        ...
        private void translateAndForward(WebSocket conn, int packetId, byte[] payload) {
            ...
            switch (packetId) {
                case 0x02: translateKeepAlive(session, payload); break;
                case 0x03: translateChatMessage(session, payload); break;
                case 0x04: translatePlayerPosition(session, payload); break;
                case 0x05: translatePlayerLook(session, payload); break;
                case 0x06: translatePlayerPositionLook(session, payload); break;
                case 0x07: translatePlayerDig(session, payload); break;
                case 0x08: translatePlayerBlockPlace(session, payload); break;
                case 0x09: translateHeldItemChange(session, payload); break;
                case 0x0A: translateAnimation(session, payload); break;
                case 0x0B: translateEntityAction(session, payload); break;
                case 0x0C: translateClickWindow(session, payload); break;
                case 0x0D: translateCloseWindow(session, payload); break;
                case 0x0E: translatePluginMessage(session, payload); break;
                default:
                    LOGGER.debug("Unhandled packet ID: 0x{}", Integer.toHexString(packetId));
            }
        }
        ...
        private void handleLoginPacket(WebSocket conn, byte[] payload) {
            try {
                ...
                // Attempt to connect the player to the default Velocity server
                Optional<RegisteredServer> targetServer = proxyServer.getServer(listenerConfig.getDefaultServer());
                if (targetServer.isPresent()) {
                    session.setTargetServer(targetServer.get());
                }
                ...
                LOGGER.warn("Failed to handle login packet", e);
        ...
        private void translateChatMessage(EaglerPlayerSession s, byte[] p) {
            ...
            if (s.getVelocityPlayer() != null) {
                s.getVelocityPlayer().spoofChatInput(msg);
            }
        }
    ...
    /**
     * Represents an active EaglerCraft player session bridging WebSocket and Velocity.
     */
    public class EaglerPlayerSession {
        ...
        private volatile Player velocityPlayer;
        ...
        public void forwardPacket(int packetId, byte[] payload) {
            ...
            // Packet forwarding handled by Velocity internals
        }
        ...
        public Player getVelocityPlayer() { return velocityPlayer; }
        public void setVelocityPlayer(Player p) { this.velocityPlayer = p; }
        public RegisteredServer getTargetServer() { return targetServer; }
        public void setTargetServer(RegisteredServer s) { this.targetServer = s; }
    }
    ...
    public static class PluginConfig {
        ...
        private String motdLine2 = "Powered by EaglercraftXVelocity";
        ...
        public static PluginConfig fromMap(Map<String, Object> map) {
            ...
            if (map.containsKey("listeners")) {
                List<Map<String, Object>> lList = (List<Map<String, Object>>) map.get("listeners");
                for (Map<String, Object> lMap : lList) {
                    c.listeners.add(ListenerConfig.fromMap(lMap));
                }
            }
        ...
        public static PluginConfig createDefault() {
            PluginConfig c = new PluginConfig();
            c.listeners.add(ListenerConfig.createDefault());
            return c;
        }
    ...
    /**
     * WebSocket listener configuration holder.
     */
    public static class ListenerConfig {
        ...
        public static ListenerConfig createDefault() { return new ListenerConfig(); }
    ...
    /**
     * /eagler command - Main administration command.
     */
    private class EaglerCommand implements SimpleCommand {
        @Override
        public void execute(Invocation invocation) {
            ...
            if (args.length == 0) {
                source.sendMessage(Component.text(
                    "EaglercraftXVelocity v" + PLUGIN_VERSION));
                source.sendMessage(Component.text(
                    "Active sessions: " + playerSessions.size()));
                source.sendMessage(Component.text(
                    "WebSocket listeners: " + wsServers.size()));
                return;
            }
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "list":
                    playerSessions.values().forEach(s ->
                        source.sendMessage(Component.text(
                            " - " + s.getUsername() + " (" + s.getUuid() + ")"))
                    );
                    break;
                case "rate":
                    source.sendMessage(Component.text(
                        "Rate limit entries: " + rateLimitMap.size()));
                    break;
                case "skins":
                    source.sendMessage(Component.text(
                        "Registered skins: " + skinRegistry.size()));
                    source.sendMessage(Component.text(
                        "Registered capes: " + capeRegistry.size()));
                    break;
                case "voice":
                    source.sendMessage(Component.text(
                ...
                case "auth":
                    source.sendMessage(Component.text(
                        "Pending auth codes: " + pendingAuthCodes.size()));
                    break;
                default:
                    source.sendMessage(Component.text(
        ...
        public boolean hasPermission(Invocation invocation) {
            return invocation.source().hasPermission("eaglercraft.admin");
        }
    }

    /**
     * /eaglerskin command - Skin and cape management.
     */
    private class EaglerSkinCommand implements SimpleCommand {
        @Override
        public void execute(Invocation invocation) {
            String[] args = invocation.arguments();
            if (args.length == 0) {
                invocation.source().sendMessage(Component.text(
                    "Usage: /eaglerskin <list|clear|reload> [username]"));
                return;
            }
            if ("list".equalsIgnoreCase(args[0])) {
                invocation.source().sendMessage(Component.text(
                    "Skins: " + skinRegistry.size() + " / Capes: " + capeRegistry.size()));
                ...
                invocation.source().sendMessage(Component.text(
                    "Cleared skin/cape for: " + args[1]));
            }
        }

        @Override
        public boolean hasPermission(Invocation invocation) {
            return invocation.source().hasPermission("eaglercraft.admin.skin");
        }
    }

    /**
     * /eaglervoice command - Voice chat management.
     */
    private class EaglerVoiceCommand implements SimpleCommand {
        @Override
        public void execute(Invocation invocation) {
            if (invocation.arguments().length == 0) {
                ...
                voiceChannels.values().forEach(ch ->
                    invocation.source().sendMessage(Component.text(
                        " - " + ch.getServerName() + " (" + ch.participants.size() + " players)"))
                );
                return;
            }
            invocation.source().sendMessage(Component.text(
                "Unknown subcommand: " + invocation.arguments()[0]));
        }

        @Override
        public boolean hasPermission(Invocation invocation) {
            return invocation.source().hasPermission("eaglercraft.admin.voice");
        }
    }
