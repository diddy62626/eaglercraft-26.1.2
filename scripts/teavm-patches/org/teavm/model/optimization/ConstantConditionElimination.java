...
// in constantTarget(). TeaVM 0.15 AGGRESSIVE optimization can create
// BranchingInstructions with null operands, causing NPE in the original code.
public class ConstantConditionElimination implements MethodOptimization {
    ...
    private BasicBlock constantTarget(Instruction instruction) {
        if (instruction instanceof BranchingInstruction) {
            ...
            Variable operand = branching.getOperand();
            if (operand == null) {
                return null;
            }
            switch (branching.getCondition()) {
                case NULL:
                    if (nullness.isNull(operand)) {
                        return branching.getConsequent();
                    } else if (nullness.isNotNull(operand)) {
                ...
                default: {
                    ...
                    if (constantDefined[operandIndex]) {
                        return checkCondition(branching.getCondition(), constants[operandIndex])
            ...
            Variable firstVar = branching.getFirstOperand();
            Variable secondVar = branching.getSecondOperand();
            if (firstVar == null || secondVar == null) {
                return null;
            }
            int first = firstVar.getIndex();
            int second = secondVar.getIndex();
            switch (branching.getCondition()) {
                case EQUAL:
                case NOT_EQUAL:
